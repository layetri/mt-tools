; Fork of marcdinkum/csd/lilypond.rkt
; Modified by Daniel Kamp
...
(define LilyGenerator%
    (class object%
           ...
           (define modgreg "mod_greg_tpl.ly")
           (define modgreg-written #f)
           ;
           ; part-specific
           ;
           ...
           (define/public (set-modgreg new-modgreg)
                          (set! modgreg new-modgreg))
           ...
           (define/public (write-modgreg)
                          (fprintf fileport "\\include ~s\n\n" modgreg))
           ...
           (define/public (write-part notes)
                          (cond
                              ...
                              ((eq? filetype 'part)
                               ...
                               (write-next-part partname notes))
                              ((eq? filetype 'score)
                               (write-version)
                               (write-header)
           ...
           (define/public (write-next-part name notes)
                          (begin
                              (fprintf fileport "  \\new Staff\n  {\n")
...
(define (lilypond-modgreg path)
    (if (equal? lilygenerator #f)
        (warning-no-lily-object)
        (send lilygenerator set-modgreg path)))
